...
import { useStocks, useAddStock, useDeleteStock, useStockInfo } from "@/hooks/useWatchlist";
...
export default function StockDetailPage() {
  ...
  // 관심종목 여부 및 정보
  const watchlistStock = useMemo(() => stocks.find((s) => s.ticker === ticker) ?? null, [stocks, ticker]);
  const isInWatchlist  = !!(watchlistStock?.is_active);
  const stockName      = watchlistStock?.name   ?? stockInfo?.name   ?? ticker;
  const stockMarket    = watchlistStock?.market  ?? stockInfo?.market  ?? "";
  const p              = watchlistStock?.latest_price ?? null;

  // 관심종목 내 prev/next 네비게이션
  ...
  const goTo = useCallback((t: string) => router.push(`/stocks/${t}`), [router]);
  ...
  const toggleWatchlist = () => {
    if (isInWatchlist) {
      ...
      addMutation.mutate({ ticker, name: stockName, market: stockMarket });
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        {stocks.length > 1 && currentIndex >= 0 && (
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {prevStock && <span className="hidden sm:inline max-w-[8rem] truncate">{prevStock.name}</span>}
            </button>
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm transition-colors bg-gray-800 disabled:opacity-30 disabled:cursor-not-allowed hover:bg-gray-700 text-gray-300 hover:text-white"
            >
              {nextStock && <span className="hidden sm:inline max-w-[8rem] truncate">{nextStock.name}</span>}
              <span className="text-lg leading-none">›</span>
            </button>
          </div>
        )}
      ...
      <div className="bg-gray-800 rounded-xl p-6">
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              <h1 className="text-2xl font-bold text-white">{stockName}</h1>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              disabled={isToggling}
              className={`text-sm px-4 py-1.5 rounded-lg transition-colors font-medium disabled:opacity-50 ${
                isInWatchlist
                  ? "bg-emerald-700 hover:bg-red-700/80 text-white"
                  : "bg-gray-700 hover:bg-emerald-700 text-gray-300 hover:text-white"
              }`}
            >
              {isToggling ? "..." : isInWatchlist ? "★ 관심종목" : "☆ 관심 추가"}
            </button>
        ...
        {p && (
          <div className="flex flex-wrap gap-6 mt-4 text-sm text-gray-400">
            {p.ma20  != null && <span>MA20  <span className="text-white font-medium">{p.ma20.toFixed(0)}</span></span>}
            {p.ma60  != null && <span>MA60  <span className="text-white font-medium">{p.ma60.toFixed(0)}</span></span>}
            {p.ma120 != null && <span>MA120 <span className="text-white font-medium">{p.ma120.toFixed(0)}</span></span>}
            {p.ma200 != null && <span>MA200 <span className="text-white font-medium">{p.ma200.toFixed(0)}</span></span>}
          </div>
        )}
      </div>
      ...
      {isInWatchlist && watchlistStock && (
        <div className="bg-gray-800 rounded-xl p-6">
          <AICommentaryCard ticker={ticker} stockName={stockName} />
        </div>
      )}
    </div>
  );
