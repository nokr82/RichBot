...
export default function DisclosureFeed() {
  ...
  const filtered = query.trim()
    ? disclosures.filter((d) => {
        ...
        return (
          d.corp_name.toLowerCase().includes(q) ||
          d.report_nm.toLowerCase().includes(q)
        );
      })
    : disclosures;
  ...
  return (
    <div className="space-y-3">
      ...
      {!isLoading && query.trim() && filtered.length > 0 && (
        <p className="text-gray-500 text-xs text-right">{filtered.length}건 검색됨</p>
      )}
    </div>
  );
}
